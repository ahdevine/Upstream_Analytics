...
#set environment
arcpy.env.workspace = r"Z:\gully_road_length\gully_road_length_project\gully_road_length.gdb"
arcpy.env.overwriteOutput = True

#import data and add corresponding feature layers
culverts = r"\\zoofiles.uvm.edu\jcampb16\gully_road_length\gully_road_length_data\monkton_data.gdb\Monkton_Culvpt"
arcpy.management.MakeFeatureLayer(culverts, "culverts_lyr")
roads = r"Z:\gully_road_length\gully_road_length_data\roads_with_elev.gdb\Monkton_Rds_Seg_Elevation"
...
with arcpy.da.SearchCursor(culverts, ["SHAPE@", "elevation"]) as iterate_cursor:
    for row in iterate_cursor:
        ...
        with arcpy.da.InsertCursor("target_culvert", ["SHAPE@", "elevation"]) as create_target_culvert_cursor:
        ...
        with arcpy.da.SearchCursor(target_roads, ["MaxEl", "Culv_El_Max", "Inter_El", "SHAPE@"]) as road_cursor:
